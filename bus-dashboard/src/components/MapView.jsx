  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      buses.forEach((bus, idx) => {
        ...
        const start = Array.isArray(bus.routeCoords) && bus.routeCoords.length ? bus.routeCoords[0] : LAHORE
            ...
            features: [
              {
                ...
                properties: { id: bus.id, name: bus.name, routeName: bus.routeName },
              },
            ],
          },
        })
...
map.on('mouseenter', pointSourceId, () => (map.getCanvas().style.cursor = 'pointer'))
map.on('mouseleave', pointSourceId, () => (map.getCanvas().style.cursor = ''))
map.on('click', pointSourceId, (e) => {
    ...
    if (btn) {
      ...
      try {
        const features = buses
          .filter((b) => Array.isArray(b.routeCoords) && b.routeCoords.length > 0)
          .map((b) => ({ type: 'Feature', geometry: { type: 'LineString', coordinates: b.routeCoords }, properties: {} }))
        if (features.length) {
  ...
  function animateBus(map, bus) {
    ...
    const step = () => {
      ...
      const distKm = Math.min(lengthKm * fraction, Math.max(0, lengthKm - 1e-6))
      let pt = null
      try {
  ...
  function setBasemap(map, key) {
    ...
    // add layer normally (no 'before' param). It will be drawn below subsequently added vector layers.
    // If you ever need to place it explicitly beneath other layers you can find an existing layer id and pass it as the before argument.
